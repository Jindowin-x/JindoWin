...
-(void)getAllProvinceList{
    [UNUrlConnection getAreaListWithID:-1 complete:^(NSDictionary *resultDic, NSString *errorString) {
        if (errorString) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 17233) {
        ...
        cityChooseVC.stopType = self.stopType;
        long parentsID = [listDic[@"id"] longValue];
        cityChooseVC.parentID = parentsID;
        if (self.resultBlock) {
            cityChooseVC.resultBlock = self.resultBlock;
        }
